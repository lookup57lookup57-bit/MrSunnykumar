    ...
    try:
        ...
        if result["pk"] and result["cs"]:
            ...
            if "error" not in init_data:
                ...
                # FIXED: Get the CORRECT total amount
                lig = init_data.get("line_item_group")
                inv = init_data.get("invoice")
                pi = init_data.get("payment_intent") or {}
                
                # Get total from payment_intent first (most reliable)
                total = pi.get("amount", 0)
                
                if total == 0 and lig:
                    total = lig.get("total", 0)
                elif total == 0 and inv:
                    total = inv.get("total", 0)
                
                # If still zero, try to calculate from line items
                if total == 0 and lig and lig.get("line_items"):
                    line_items = lig.get("line_items", [])
                    for item in line_items:
                        item_amount = item.get("amount", 0) * item.get("quantity", 1)
                        total += item_amount
                
                result["price"] = total / 100 if total > 0 else 0
                
                # Get currency
                if pi.get("currency"):
                    result["currency"] = pi.get("currency", "").upper()
                elif lig and lig.get("currency"):
                    result["currency"] = lig.get("currency", "").upper()
                elif inv and inv.get("currency"):
                    result["currency"] = inv.get("currency", "").upper()
                
                # FIXED: Better product display with proper amounts
                if lig and lig.get("line_items"):
                    ...
                    for item in items:
                        ...
                        if interval:
                            product_parts.append(f"{qty} × {name} ({sym}{amt:.2f} / {interval})")
                        else:
                            product_parts.append(f"{qty} × {name} ({sym}{item_total:.2f})")
                    
                    result["product"] = ", ".join(product_parts)
                    
                    # If total from items doesn't match, show both
                    if total_calc * 100 != total and total > 0:
                        result["product"] += f" + fees/tax (Total: {sym}{total/100:.2f})"
                else:
                    # Try to create product from description
                    desc = init_data.get("description") or init_data.get("statement_descriptor")
                    if desc:
                        result["product"] = desc
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            async with aiohttp.ClientSession(connector=connector) as s:
                ...
                # If no payment_intent, try line_item_group or invoice
                if total == 0 and lig:
                    total = lig.get("total", 0)
                ...
                print(f"[DEBUG] Total amount: {total} ({total/100} {checkout_data.get('currency', '')})")
                
                if total <= 0:
                    result["status"] = "FAILED"
                    result["response"] = "Invalid amount (zero or negative)"
                    result["time"] = round(time.perf_counter() - start, 2)
                    return result
                ...
                if "error" in pm:
                    ...
                    if "currency" in err_msg.lower() or "amount" in err_msg.lower():
                        result["status"] = "CURRENCY ERROR"
                        result["response"] = err_msg
                    elif "unsupported" in err_msg.lower() or "tokenization" in err_msg.lower():
                ...
                # FIXED: Always include amount in confirmation
                ...
                if "error" in conf:
                    ...
                    # FIXED: Better decline code handling
                    if "amount" in msg.lower() or "currency" in msg.lower():
...
async def co_handler(msg: Message):
    ...
    if not cards:
        # FIXED: Get currency and price properly
        currency = checkout_data.get('currency', '')
        if not currency and checkout_data.get('init_data'):
            init = checkout_data['init_data']
            lig = init.get('line_item_group', {})
            inv = init.get('invoice', {})
            pi = init.get('payment_intent', {})
            currency = (lig.get('currency') or inv.get('currency') or pi.get('currency') or '').upper()
        
        price = checkout_data.get('price', 0)
        if price == 0 and checkout_data.get('init_data'):
            init = checkout_data['init_data']
            lig = init.get('line_item_group', {})
            inv = init.get('invoice', {})
            pi = init.get('payment_intent', {})
            price = (lig.get('total') or inv.get('total') or pi.get('amount') or 0) / 100
        
        sym = get_currency_symbol(currency)
        price_str = f"{sym}{price:.2f} {currency}" if price and price > 0 else f"{sym}0.00 {currency or 'USD'}"
        
        # FIXED: Product display
        product_display = checkout_data.get('product', 'N/A')
        if product_display == 'N/A' and checkout_data.get('init_data'):
            init = checkout_data['init_data']
            lig = init.get('line_item_group', {})
            if lig and lig.get('line_items'):
                items = lig['line_items']
        ...
        response += f"「❃」 𝗣𝗿𝗼𝗱𝘂𝗰𝘁 : <code>{product_display}</code>\n"
    ...
    # FIXED: Get currency and price for charging display
    currency = checkout_data.get('currency', '')
    if not currency and checkout_data.get('init_data'):
        init = checkout_data['init_data']
        lig = init.get('line_item_group', {})
    ...
    for i, card in enumerate(cards):
        ...
        if len(cards) > 1 and (time.perf_counter() - last_update) > 1.5:
            ...
            errors = sum(1 for r in results if r['status'] in ['ERROR', 'FAILED', 'CURRENCY ERROR', 'AMOUNT ERROR'])
    ...
    # FIXED: Final response with proper price
    ...
    if charged_card:
        ...
        if r['status'] == '3DS':
            ...
            status_emoji = "🚫"
        elif r['status'] in ['CURRENCY ERROR', 'AMOUNT ERROR']:
            status_emoji = "💰"
        else:
            status_emoji = "⚠️"
        ...
        declined = sum(1 for r in results if r['status'] == 'DECLINED')
        three_ds = sum(1 for r in results if r['status'] in ['3DS', '3DS SKIP'])
        errors = sum(1 for r in results if r['status'] in ['ERROR', 'FAILED', 'UNKNOWN', 'CURRENCY ERROR', 'AMOUNT ERROR'])
        total = len(results)
